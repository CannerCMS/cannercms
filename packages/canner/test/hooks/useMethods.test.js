...
beforeEach(() => {
  ...
  Com = ({ pattern }) => {
    const {
      rootValue, request, deploy, reset,
    } = useMethods({ pattern });
    ...
    return (
      <div data-testid="mock-com">
        ...
        <button type="button" data-testid="mock-update-title" onClick={updateTitle}>update title</button>
        <button type="button" data-testid="mock-deploy" onClick={() => deploy('posts')}>deploy</button>
        <button type="button" data-testid="mock-reset" onClick={() => reset('posts')}>reset</button>
      </div>
    );
  };
  routes = ['posts'];
